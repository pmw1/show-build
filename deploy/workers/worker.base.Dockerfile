...
# nfs-common = self-mount the media share at first run; gosu = drop root->worker
# after mounting; util-linux provides mountpoint(1).
...
# Worker user (uid 1001 / group disaffected). The container RUNS AS ROOT so the
# entrypoint can mount NFS; the entrypoint drops to this user (gosu) for celery.
...
# NOTE: do NOT `USER insider` — entrypoint needs root to mount(), then gosu-drops.
