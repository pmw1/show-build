...
def validate_cue_blocks(segment_files):
    ...
    for file_path in segment_files:
        ...
        for block in cue_blocks:
            ...
            # ADLIB cues don't require slug, other cues do
            if cue_type_str != "ADLIB":
                if not slug or not slug.group(1).strip():
                    errors.append("Missing or empty [Slug]")
            
            # For inline ADLIB, AssetID is optional; for other cues it's required
            if not adlib_inline and cue_type_str != "ADLIB":
...
def process_segment(file_path, para_counter):
    ...
    # New approach: split on actual cue blocks and SOT patterns, preserving content
    cue_and_sot_pattern = r"(<!--\s*Begin Cue\s*-->.*?<!--\s*End Cue\s*-->|\{SOT/[^}]+\})"
    parts = re.split(cue_and_sot_pattern, content, flags=re.DOTALL | re.IGNORECASE)
    ...
    for part in parts:
        ...
        # Check if this part is a complete cue block
        if re.match(r"<!--\s*Begin Cue\s*-->.*<!--\s*End Cue\s*-->", part, re.DOTALL | re.IGNORECASE):
            # Process cue block
        ...
        # Check if this part is a SOT reference
            ...
            output.append(
                "<div class='cue'>\n"
                f"<p class='cue-header' style='font-weight:bold; font-size:1em;'>[[ SOT / {sot_name} ]]</p>\n"
                "<div class='sot'>\n"
            ...
            # This is regular content - process as paragraphs
            paragraphs = part.split("\n\n")
            for para in paragraphs:
                para = para.strip()
                if not para or re.match(r"\[MediaURL:[^\]]+\]", para, re.IGNORECASE):
                    continue
                # Skip standalone "Script" text (common in empty template sections)
                if para.lower() == "script":
                    continue
...
def compile_script(episode_number, validate_only=False):
    ...
    scripts_path = episode_path / "scripts"
    scripts_path.mkdir(exist_ok=True)  # Create scripts directory if it doesn't exist
    output_path = scripts_path / base_filename
    version = 1
    while output_path.exists():
        version += 1
        output_path = scripts_path / f"script-{'-'.join(filter(None, [mm, dd, yy]))}-{status}-{version}.html"
    ...
    script_output.append(
        "<!DOCTYPE html>\n<html>\n<head>\n"
        "<style>\n"
        "body { font-family: Helvetica, sans-serif; font-size: 27px; }\n"
    ...
    for i, segment in enumerate(segment_files):
        # Add block header for each segment
        if i > 0:  # Add break before segment (except first one)
            script_output.append("<hr style='border: 2px solid #333; margin: 3em 0;' />")
        ...
        try:
            with open(segment, 'r', encoding='utf-8') as f:
                content = f.read()
            frontmatter = parse_yaml_frontmatter(content)
            segment_title = frontmatter.get('title', segment.stem)
            segment_slug = frontmatter.get('slug', segment.stem)
        except:
            segment_title = segment.stem
            segment_slug = segment.stem

        # Add block header
        script_output.append(
            f"<div style='text-align: center; margin: 2em 0; padding: 1em; background: #f0f0f0; border: 2px solid #333;'>\n"
            f"<h2 style='margin: 0; font-size: 1.8em; font-weight: bold; text-transform: uppercase;'>BLOCK: {segment_title.upper()}</h2>\n"
            f"<p style='margin: 0.5em 0 0 0; font-size: 1.2em; color: #666;'>Slug: {segment_slug}</p>\n"
            f"</div>\n"
        )
