...
export default {
  ...
  data() {
    return {
      ...
      htmlContent: '',
      episodeInfo: null,
      closeButtonState: 'normal', // 'normal' or 'warning'
      closeButtonTimer: null,
      isInverted: false,
      isFullscreen: false,
      fontSizeMultiplier: 1.0, // Base = 1.0, range: 0.5 to 2.0 (0.5pt = ~0.026 change)
      textColor: 'white' // 'white' or 'yellow'
    }
  },

  computed: {
    episodeNumber() {
      return this.$route.params.episodeNumber
    },
    ...
    sanitizedContent() {
      ...
      return this.transformForIpad(this.htmlContent)
    }
  },
  ...
  methods: {
    ...
    increaseFontSize() {
      ...
      const increment = 0.1
      this.fontSizeMultiplier = Math.min(2.5, this.fontSizeMultiplier + increment)
      this.applyFontSize()
      console.log('Font size increased to:', this.fontSizeMultiplier)
    },

    decreaseFontSize() {
      // Decrease by 1pt equivalent (using 0.1 for 2pt visible change)
      const decrement = 0.1
      this.fontSizeMultiplier = Math.max(0.5, this.fontSizeMultiplier - decrement)
      this.applyFontSize()
      console.log('Font size decreased to:', this.fontSizeMultiplier)
    },

    toggleTextColor() {
      // Toggle between white and yellow text
      this.textColor = this.textColor === 'white' ? 'yellow' : 'white'
      this.applyTextColor()
      console.log('Text color changed to:', this.textColor)
    },
    ...
    applyTextColor() {
      ...
      if (container) {
        ...
        if (scriptContent) {
          ...
          if (this.textColor === 'yellow') {
            scriptContent.style.setProperty('color', '#ffeb3b', 'important')
          } else {
            scriptContent.style.removeProperty('color')
          }
        }
      }
    },
    ...
    applyFontSize() {
      ...
      if (container) {
        ...
        if (scriptContent) {
          ...
          scriptContent.style.setProperty('--font-size-multiplier', this.fontSizeMultiplier)

          // Also set font-size directly on the injected HTML elements
          const bodyElement = scriptContent.querySelector('body')

          if (bodyElement) {
            // Apply multiplier to body font-size
            bodyElement.style.fontSize = `calc(var(--base-font-size) * ${this.fontSizeMultiplier})`
          } else {
            // If no body element, apply to content container
            scriptContent.style.fontSize = `calc(var(--base-font-size, 19px) * ${this.fontSizeMultiplier})`
          }

          console.log('Applied font size multiplier:', this.fontSizeMultiplier)
        }
      }
    },
    ...
    async loadScript() {
      ...
      try {
        ...
        const response = await axios.get(
          `/api/scripts/ipad-scroll/${this.episodeNumber}`,
          {
            headers: {
        ...
        if (response.data.success) {
          this.htmlContent = response.data.html_content
          this.episodeInfo = response.data.episode_info
          ...
          this.error = 'Failed to load script content'
        }
      } catch (err) {
        console.error('Error loading iPad scroll content:', err)
        this.error = err.response?.data?.detail || err.message || 'Unknown error'
      } finally {
        this.loading = false
      }
    },

    transformForIpad(html) {
