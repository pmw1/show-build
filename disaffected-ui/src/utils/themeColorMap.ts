...
export interface VuetifyThemeInstance {
  theme: {
    themes: {
...
let currentColors: ColorMap = { ...defaultColors }

try {
  const storedColors = localStorage.getItem('themeColors')
  if (storedColors) {
    const parsedColors = JSON.parse(storedColors) as ColorMap
...
export const updateColor = (type: string, newColor: string): boolean => {
  try {
    const updatedColors: ColorMap = { ...currentColors, [type.toLowerCase()]: newColor }
...
export const getAllColors = (): ColorMap => {
  return { ...currentColors }
}

export const resolveVuetifyColor = (
  colorName: string, 
  vuetifyInstance?: VuetifyThemeInstance, 
