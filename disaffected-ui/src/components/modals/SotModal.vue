<template>
  <v-dialog :model-value="show" @update:model-value="$emit('update:show', $event)" max-width="600">
    <v-card>
      <v-card-title>Add SOT (Sound on Tape)</v-card-title>
      <v-card-text>
        <!-- SOT fields will be populated based on user's next message -->
        <p class="text-caption">SOT modal fields coming soon...</p>
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
