"""
File Inventory Scanner Service
Scans episode directories and matches files to canonical expectations using LLM analysis.
"""
...
CANONICAL_EXPECTATIONS = [
    ...
    # Thumbnail Masters
    {
        ...
        "id": "thumbnail.master_widescreen_source",
        "category": "thumbnail",
    ...
    },
    {
        ...
        "id": "thumbnail.master_widescreen_export",
        "category": "thumbnail",
    ...
    },
    {
        ...
        "id": "thumbnail.master_square_source",
        "category": "thumbnail",
    ...
    },
    {
        ...
        "id": "thumbnail.master_square_export",
        "category": "thumbnail",
    ...
    {
        "id": "project.ae_teaser",
        "category": "project",
        "purpose": "Guest interview teaser production project",
        "characteristics": "AEP file (After Effects), contains teaser and guest name",
        "location_hints": "projects/teasers/{EPISODE}-teaser-{guest}/*.aep",
        "required": False,
        "multiplicity": "multiple"
    },

    # Assets (generic, will match many files)
    {
        "id": "asset.video",
        "category": "asset",
        "purpose": "Final video clip used in show",
        "characteristics": "MP4 or MOV, in assets/video folder, moderate file size",
        "location_hints": "assets/video/*.mp4, assets/video/*.mov",
        "required": False,
        "multiplicity": "multiple"
    },
    {
...
def scan_directory_recursive(base_path: Path, exclude_dirs: List[str] = None) -> List[Dict[str, Any]]:
    ...
    Args:
        ...
        exclude_dirs: List of directory names to skip (.git, .syncthing, etc.)
...
async def match_files_to_expectations(
    ...
    """
    Match scanned files to canonical expectations using LLM.

    Args:
        files: List of file metadata from scan
        episode_number: Episode number for substitution
        llm_service: LLM service for making requests
        db_session: Database session for prompt overrides
        confidence_threshold: Minimum confidence to accept match (0-100)

    Returns:
        Tuple of (matched_expectations, unmatched_files)
    """
    matched = []
...
async def classify_unmatched_files(
    ...
    """
    Classify files that didn't match any canonical expectation.

    Args:
        files: List of unmatched file metadata
        llm_service: LLM service
        db_session: Database session for prompt overrides

    Returns:
        List of classified stray files
    """
    classified = []
...
def generate_mapping_file(
    ...
    """
    Generate UFDP-format YAML mapping file.

    Args:
        episode_number: Episode number
        matched: List of matched expectations
        stray_files: List of classified stray files
