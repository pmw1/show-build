...
This document defines the **ONLY** acceptable directory structure and naming conventions for episode storage in the Show-Build system.

**Canonical Source of Truth**: `/mnt/sync/disaffected/episodes/{EPISODE}/`

### Principles

1. **Local Syncthing is Canonical**: Episode folders in `/mnt/sync/disaffected/episodes/` are the single source of truth
2. **Google Drive is Mirror/Archive**: Google Drive serves as backup and archive, mirroring local structure
3. **Database Syncs to Files**: Database content generates file artifacts (scripts, rundowns, etc.)
4. **Structure is Enforced**: Show-Build will not create non-compliant structures
5. **Manual Creation Prohibited**: Unless explicitly directed by admin, directories are created by Show-Build only
...
**Required Fields**:
- `episode`: 4-digit episode number
- `episode_asset_id`: Server-generated AssetID
- `title`: Episode title
- `air_date`: YYYY-MM-DD format
...
**Purpose**: Rundown organization and media playlist generation
...
```
rundown/
└── media-list/           # vMix playlist (symlinks to assets)
    ├── 010-010-SOT-...
    ├── 010-020-GFX-...
    └── ...
```
